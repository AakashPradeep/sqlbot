...
### Database username/password?
	...
	•	For Postgres/MySQL, embed credentials in DB_URL:
	•	postgresql+psycopg2://USER:PASSWORD@HOST:5432/DBNAME
	•	mysql+pymysql://USER:PASSWORD@HOST:3306/DBNAME

Tip: If you don’t want passwords in .env, you can also use cloud IAM auth / local secrets manager later. For MVP, .env is fine.

⸻

## 🛡️ Policy file (policy.yaml)
...
### How allow/deny works
	•	If deny_* matches → blocked (even if allowed)
	•	If no allow rules exist (both allow lists empty) → fail closed
	•	Otherwise → query must match allow rules
...
Examples
	•	“Show revenue over time as a line chart”
	•	“Plot top 10 customers by spend”
	•	“Show scatter plot of quantity vs total”

Controls
	•	Output mode
	•	auto: chart only when you ask
	•	chart: always show chart (even if user didn’t ask)
	•	table_only: never show chart
	•	Chart type
	•	auto / line / bar / scatter / area / box
	•	X / Y overrides
	•	if the auto-picked columns aren’t what you want

⸻

## 🧪 Testing with Chinook (SQLite)

If you’re using the sample SQLite DB:
	1.	Ensure it exists:
    ```
...
If you see: unable to open database file
	...
	•	ensure DB_URL=sqlite:///examples/chinook.db
	•	run from repo root (same directory as src/)

⸻

## 🔎 How it works (high-level flow)
	1.	Load policy → compile regex once
	2.	Introspect schema → convert to prompt-friendly text
	...
	8.	If the user asks for a chart → infer x/y + render a single plot

⸻

## 🔐 Security / Privacy
	•	Your DB is queried directly from your machine/server where the app runs.
	•	The LLM receives:
	•	the question
	•	schema text (introspected)
	•	policy constraints
	•	(on retry) SQL + error message
	•	If using a hosted model (OpenAI), the prompt and schema are sent to the model API.

## Recommendations
	•	Use strict allow/deny rules for production
	•	Use required tenant filters for multi-tenant setups
	•	Consider schema minimization (only include relevant tables)

⸻

## 🗺️ Roadmap / Improvements
	•	Smarter schema compression for large DBs
	•	Column-level allow/deny (PII protection)
	•	CTE-aware policy enforcement (already AST-based, can tighten more)
	•	Query cost estimation / execution timeout
	•	Cached schema + incremental refresh
	•	Local / offline LLM mode
...
## 🙌 Acknowledgements
	•	sqlglot￼ for reliable SQL parsing and AST transformations
	•	Gradio / Streamlit for quick UI iteration
